...
void mobius(){//筛法求预处理莫比乌斯函数和几个要用到的数组
    ...
    for(ll i=2;i<=maxn-5;i++){
        ...
        for(ll j=0;j<cnt&&i*pri[j]<=maxn-5;j++){
            ...
            if(i%pri[j]==0) { 
                sumu[i*pri[j]]=0;
                sumu0[i*pri[j]]=sumu0[i]%mod;
                break; 
            }
            else {
                sumu[i*pri[j]]=((-sumu[i])%mod+mod)%mod;
                sumu0[i*pri[j]]=(sumu0[pri[j]]*sumu0[i])%mod;
            }
        }
    }
    for(ll i=1;i<=maxn-5;i++) {
        sumu[i]=(sumu[i-1]+sumu[i])%mod;
        f0[i]=g0[i]=mod+1;
        sumhe[i]=(sumhe[i-1]+i)%mod;
        sumu0[i]=(sumu0[i-1]+i*sumu0[i]%mod)%mod;
    }
}
